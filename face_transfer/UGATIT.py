...
class UGATIT(object) :
    ...
    def load_test_model(self):
        self.saver = tf.train.Saver()
        could_load, checkpoint_counter = self.load(self.checkpoint_dir)
        self.could_load = could_load

    def test(self,test_image_path=None, **kwargs):
        # sess=kwargs['sess']
        ...
        print('Processing human image to anime: ' + test_image_path)
        sample_image = np.asarray(load_test_data(test_image_path, size=self.img_size))
