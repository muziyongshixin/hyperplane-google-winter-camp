...
def main(content_image_path=None):
    ...
    for i, sample in enumerate(dataloader):
        ...
        if args.stylePair:
            log.info('content: ' + str(sample['contentPath']) + '\tstyle 1: ' + str(
                sample['style0Path']) + '\tstyle 2: ' + str(sample['style1Path']))
            ...
            if args.synthesis:
                for ii in range(1, 4):
                    ...
                    result_path = save_image(content, 'texture_iter' + str(ii), s0_basename + '_and_' + s1_basename,
                                             str(s0_ext),
                                             args)
            else:
                ...
                result_path = save_image(out, c_basename, s0_basename + '_and_' + s1_basename, c_ext, args)
        else:
            ...
            if args.synthesis:
                for ii in range(1, 4):
                    ...
                    result_path = save_image(content, 'texture_iter' + str(ii), s_basename, s_ext, args)
            else:
                c_basename = str(os.path.basename(sample['contentPath'][0]).split('.')[0])
                c_ext = str(os.path.basename(sample['contentPath'][0]).split('.')[-1])
                ...
                result_path = save_image(out, c_basename, s_basename, c_ext, args)
                # return result_path
